  ...
  try {
    ...
    if (user.isRequesting) {
      if (isNumber(userMessage)) {
        ...
        reply.push(`[候位資訊]\n\n候位號碼：${assignedNumber}\n用餐人數：${groupSize}\n發起時間：${strftime('%l:%M%P %e %b %Y', new Date(newReq.requestMadeTime + 480 * 60000))}`);
      ...
      if (user.isWaiting) {
        ...
        await updateDoc(doc(db, 'todayWaitRequests', user.waitRequestId), { isWaiting: false, status: 'canceled', cancelTime: Date.now() });
